...
- `/<game_id>/<variant_id>/` : 
    - Returns information about the variant specified by `variant_id` of the game specified by `game_id`. This is used, for example, by GamesmanUni when a game is clicked in order to see which variants are available in order to render the list of available variants.
...
- `/<game_id>/<variant_id>/positions/?p=<position_string>` : 
    ...
    - Returns information about the position specified by `position_string` of the variant specified by `variant_id` of the game specified by `game_id`. This is used, for example, by GamesmanUni when it needs to load a new position every time a user makes a move.
      ...
      - `position`: (String) The human-readable string representation the position (e.g., a FEN string in Chess).
      ...
      - `positionValue`: (String) The value of this position (one of "win", "lose", "tie", or "draw"). There is also the value "unsolved" if the value of this position is unknown.
      ...
      - `moves`: (array): A list of move objects, which contain the following fields:
        ...
        - `moveValue`: (String) The outcome of the game for the player-to-move if they make this move and both players play perfectly afterward.
        - `deltaRemoteness`: (Number) Roughly speaking, the value of this is lower if the remoteness is better based on what the moveValue is. GamesmanUni uses this information to set different opacities for move buttons corresponding to moves of the same value but different remoteness.
...
- `/<game_id>/<variant_id>/instructions/?language=<language>` : 
